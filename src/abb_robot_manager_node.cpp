...
#include "solution_manager_core/controller_names.hpp"
#include "solution_manager_core/robot_manager_node.hpp"
...
{
RobotManagerNode::RobotManagerNode() : solution_manager_core::ROS2BaseLCNode("robot_manager")
{
  ...
  this->registerStaticParameter<std::string>(
    "robot_model", "ABBMultiInterfaceHardware", solution_manager_core::ParameterSetAccessRights{false, false},
...
RobotManagerNode::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  if (!solution_manager_core::changeControllerState(
        change_controller_state_client_,
        {solution_manager_core::JOINT_STATE_BROADCASTER,
         solution_manager_core::JOINT_TRAJECTORY_CONTROLLER},
        {}))
  {
...
bool RobotManagerNode::onRobotModelChangeRequest(const std::string & robot_model)
{
  ...
  // [Modification]: We do not need to rename the robot_model because the UR XACRO is from official repo. - Zhihao Liu
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<solution_manager_core::RobotManagerNode>();
