...
def launch_setup(context, *args, **kwargs):
    ...
    print(f"Hardware Component Name: {tf_prefix + robot_model.perform(context)}")
    control_node = Node(
        namespace=ns,
...
def generate_launch_description():
    ...
    launch_arguments.append(DeclareLaunchArgument("namespace", default_value="ips_kuka"))
    launch_arguments.append(DeclareLaunchArgument("client_port", default_value="59152"))
    launch_arguments.append(DeclareLaunchArgument("client_ip", default_value="192.168.2.86"))
    ...
    launch_arguments.append(
        DeclareLaunchArgument(
            "controller_config",
            default_value=get_package_share_directory("dtir_solution_manager")
            + "/config/kuka/ros2_controller_config.yaml",
        )
    )
    launch_arguments.append(
        DeclareLaunchArgument(
            "jtc_config",
            default_value=get_package_share_directory("dtir_solution_manager")
            + "/config/kuka/joint_trajectory_controller_config.yaml",
        )
    )
